...
After INDEX
"Sort  (cost=9.12..9.15 rows=13 width=55) (actual time=0.300..0.302 rows=5.00 loops=1)"
"  Sort Key: (count(DISTINCT a.instance_id)) DESC"
"  Sort Method: quicksort  Memory: 25kB"
"  Buffers: shared hit=4"
"  ->  GroupAggregate  (cost=7.88..8.88 rows=13 width=55) (actual time=0.284..0.296 rows=5.00 loops=1)"
"        Group Key: e.employment_id, (concat(TRIM(BOTH FROM p.first_name), ' ', p.last_name))"
"        Filter: (count(DISTINCT a.instance_id) > 1)"
"        Buffers: shared hit=4"
"        ->  Sort  (cost=7.88..7.98 rows=40 width=51) (actual time=0.277..0.280 rows=40.00 loops=1)"
"              Sort Key: e.employment_id, (concat(TRIM(BOTH FROM p.first_name), ' ', p.last_name)), a.instance_id"
"              Sort Method: quicksort  Memory: 27kB"
"              Buffers: shared hit=4"
"              ->  Hash Join  (cost=4.03..6.82 rows=40 width=51) (actual time=0.126..0.201 rows=40.00 loops=1)"
"                    Hash Cond: (e.person_id = p.person_id)"
"                    Buffers: shared hit=4"
"                    ->  Hash Join  (cost=2.56..4.92 rows=40 width=23) (actual time=0.080..0.113 rows=40.00 loops=1)"
"                          Hash Cond: (a.worker_id = e.worker_id)"
"                          Buffers: shared hit=3"
"                          ->  Hash Join  (cost=1.08..3.32 rows=40 width=12) (actual time=0.046..0.069 rows=40.00 loops=1)"
"                                Hash Cond: (a.instance_id = ci.instance_id)"
"                                Buffers: shared hit=2"
"                                ->  Seq Scan on allocation a  (cost=0.00..1.80 rows=80 width=8) (actual time=0.021..0.025 rows=80.00 loops=1)"
"                                      Buffers: shared hit=1"
"                                ->  Hash  (cost=1.06..1.06 rows=2 width=8) (actual time=0.014..0.014 rows=2.00 loops=1)"
"                                      Buckets: 1024  Batches: 1  Memory Usage: 9kB"
"                                      Buffers: shared hit=1"
"                                      ->  Seq Scan on course_instance ci  (cost=0.00..1.06 rows=2 width=8) (actual time=0.011..0.012 rows=2.00 loops=1)"
"                                            Filter: ((study_year = 2025) AND (study_period = 'P2'::study_period_enum))"
"                                            Rows Removed by Filter: 2"
"                                            Buffers: shared hit=1"
"                          ->  Hash  (cost=1.21..1.21 rows=21 width=19) (actual time=0.025..0.025 rows=21.00 loops=1)"
"                                Buckets: 1024  Batches: 1  Memory Usage: 10kB"
"                                Buffers: shared hit=1"
"                                ->  Seq Scan on employee e  (cost=0.00..1.21 rows=21 width=19) (actual time=0.011..0.015 rows=21.00 loops=1)"
"                                      Buffers: shared hit=1"
"                    ->  Hash  (cost=1.21..1.21 rows=21 width=61) (actual time=0.029..0.029 rows=21.00 loops=1)"
"                          Buckets: 1024  Batches: 1  Memory Usage: 10kB"
"                          Buffers: shared hit=1"
"                          ->  Seq Scan on person p  (cost=0.00..1.21 rows=21 width=61) (actual time=0.016..0.020 rows=21.00 loops=1)"
"                                Buffers: shared hit=1"
"Planning:"
