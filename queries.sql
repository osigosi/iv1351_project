...
BEFORE TESRING
"Sort  (cost=8.85..8.89 rows=13 width=55) (actual time=0.496..0.500 rows=5.00 loops=1)"
"  Sort Key: (count(DISTINCT a.instance_id)) DESC"
"  Sort Method: quicksort  Memory: 25kB"
"  Buffers: shared hit=4"
"  ->  GroupAggregate  (cost=7.68..8.61 rows=13 width=55) (actual time=0.466..0.489 rows=5.00 loops=1)"
"        Group Key: e.employment_id, (concat(p.first_name, ' ', p.last_name))"
"        Filter: (count(DISTINCT a.instance_id) > 1)"
"        Buffers: shared hit=4"
"        ->  Sort  (cost=7.68..7.78 rows=40 width=51) (actual time=0.451..0.458 rows=40.00 loops=1)"
"              Sort Key: e.employment_id, (concat(p.first_name, ' ', p.last_name)), a.instance_id"
"              Sort Method: quicksort  Memory: 28kB"
"              Buffers: shared hit=4"
"              ->  Hash Join  (cost=4.03..6.62 rows=40 width=51) (actual time=0.198..0.293 rows=40.00 loops=1)"
"                    Hash Cond: (e.person_id = p.person_id)"
"                    Buffers: shared hit=4"
"                    ->  Hash Join  (cost=2.56..4.92 rows=40 width=23) (actual time=0.124..0.183 rows=40.00 loops=1)"
"                          Hash Cond: (a.worker_id = e.worker_id)"
"                          Buffers: shared hit=3"
"                          ->  Hash Join  (cost=1.08..3.32 rows=40 width=12) (actual time=0.072..0.111 rows=40.00 loops=1)"
"                                Hash Cond: (a.instance_id = ci.instance_id)"
"                                Buffers: shared hit=2"
"                                ->  Seq Scan on allocation a  (cost=0.00..1.80 rows=80 width=8) (actual time=0.029..0.037 rows=80.00 loops=1)"
"                                      Buffers: shared hit=1"
"                                ->  Hash  (cost=1.06..1.06 rows=2 width=8) (actual time=0.027..0.028 rows=2.00 loops=1)"
"                                      Buckets: 1024  Batches: 1  Memory Usage: 9kB"
"                                      Buffers: shared hit=1"
"                                      ->  Seq Scan on course_instance ci  (cost=0.00..1.06 rows=2 width=8) (actual time=0.020..0.021 rows=2.00 loops=1)"
"                                            Filter: ((study_year = 2025) AND (study_period = 'P2'::study_period_enum))"
"                                            Rows Removed by Filter: 2"
"                                            Buffers: shared hit=1"
"                          ->  Hash  (cost=1.21..1.21 rows=21 width=19) (actual time=0.039..0.039 rows=21.00 loops=1)"
"                                Buckets: 1024  Batches: 1  Memory Usage: 10kB"
"                                Buffers: shared hit=1"
"                                ->  Seq Scan on employee e  (cost=0.00..1.21 rows=21 width=19) (actual time=0.016..0.023 rows=21.00 loops=1)"
"                                      Buffers: shared hit=1"
"                    ->  Hash  (cost=1.21..1.21 rows=21 width=61) (actual time=0.054..0.054 rows=21.00 loops=1)"
"                          Buckets: 1024  Batches: 1  Memory Usage: 10kB"
"                          Buffers: shared hit=1"
"                          ->  Seq Scan on person p  (cost=0.00..1.21 rows=21 width=61) (actual time=0.032..0.038 rows=21.00 loops=1)"
"                                Buffers: shared hit=1"
"Planning:"
